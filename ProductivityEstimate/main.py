...
def add_productivity_at_time(hour, i_quarter_of_hour, typing_num_added, mouse_move_added):
    output_typing_num[hour][i_quarter_of_hour] += typing_num_added
    output_mouse_move[hour][i_quarter_of_hour] += mouse_move_added


def convert_input_to_output(start_hour, start_minute, duration_sec, typing_num, mouse_move):
    ...
    if (start_minute % interval_min != 0):
        ...
        if (remain_in_this_quarter_sec >= duration_sec):
            add_productivity_at_time(start_hour, i_quarter_of_hour, typing_num, mouse_move)
            duration_sec = 0
        else:
            add_productivity_at_time(start_hour, i_quarter_of_hour,
...
def output_to_csv():
    ...
    for hour in range(max_hour):
        for i_quarter in range(4):
            ...
            line += str(output_typing_num[hour][i_quarter]) + "," + str(output_mouse_move[hour][i_quarter])
            line += '\n'
            output_content += line
    ...
    with open(input_file_path) as csv_file:
        ...
        for row in csv_reader:
            if is_header:
                ...
                convert_input_to_output(int(hour), int(minute), int(row[i_col_duration_sec]), int(row[i_col_type_num]),
